...
impl PoHCertificate {
    ...
    pub fn to_cbor_signable(&self) -> Result<Vec<u8>> {
        ...
        // 4: trust_score
        map.push((Value::Integer(4.into()), Value::Integer((self.trust_score as i128).into())));

        // 5: confidence
        map.push((Value::Integer(5.into()), Value::Float(self.confidence)));

        // 6: chain_length
        map.push((Value::Integer(6.into()), Value::Integer((self.chain_length as i128).into())));

        // 7: unique_cells
        map.push((Value::Integer(7.into()), Value::Integer((self.unique_cells as i128).into())));
        ...
        map.push((Value::Integer(10.into()), Value::Integer((self.issued_at.timestamp() as i128).into())));

        // 11: valid_seconds
        map.push((Value::Integer(11.into()), Value::Integer((self.valid_seconds as i128).into())));
    ...
    pub fn to_cbor(&self) -> Result<Vec<u8>> {
        let mut signable = self.to_cbor_signable()?;
